...
class Pipeline:
	...
	def get_links(self):
		...
		if self.search_keywords == 'True':
			...
			if len(video_ids) > int(self.number_of_links):
				video_ids = video_ids[:int(self.number_of_links)]
	...
	def get_captionpath(self, yt):
		...
		caption_path = join(self.dataset_path, self.parse_title(yt.title))
		caption_path = join(caption_path, f'captions.xml')
	...
	def get_videopath(self, yt):
		try:
			...
			video_path = join(self.dataset_path, self.parse_title(yt.title))
			video_stream.first().download(video_path)
			video_path = join(video_path, 'video.mp4')
			print(video_path)
	...
	def get_audiopath(self, yt):
		try:
			...
			audio_path = join(self.dataset_path, self.parse_title(yt.title))
			audio_path = join(audio_path, 'audio.mp4')
	...
	def get_extracted_text(self, video_path, title, link):
		...
		extracted_text_path = join(self.dataset_path, f'{title}', f'text.txt')
	...
	def download_data(self, link):
		...
		try:
			...
			if not self.data_dict[link]:
				# save video title
				video_title = self.parse_title(yt.title)
				try:
					mkdir(join(self.dataset_path, video_title))
				except:
					print('video and contents already processed')
				...
				#self.sort_to_topic(caption_path)
	...
	# Parse Video title to remove incompatible characters from filenames
	def parse_title(self, title):
		regex = re.compile('[,|\/.!?:;*@(){}]')
		return regex.sub('', title).strip()
